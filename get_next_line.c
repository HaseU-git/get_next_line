...
/*   Updated: 2020/12/23 21:54:30 by yhasegaw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_next_line(int fd, char **line)
{
	...
	if (check_error(fd, line, &keeper))
		return (-1);
	if (check_endl(keeper))
		return (create_ans(&keeper, line));
	else
		read_ret = read_section(fd, &keeper, line);
	if (read_ret < 0)
		return (safe_free(line, &keeper, -1));
	if (check_endl(keeper))
		return (create_ans(&keeper, line));
